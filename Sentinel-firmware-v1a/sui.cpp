...
static const size_t MAX_PAYLOAD = 600;  // comfortably covers a simple-transfer PTB
...
bool derivePub(const uint8_t *entropy, size_t entLen, const char *passphrase,
  ...
  m = "";
  uint32_t path[5] = { 44, 784, 0, 0, index };  // SLIP-10 hardens each element
...
String address(const uint8_t *entropy, size_t entLen,
  ...
  // "0x" + Blake2b-256(flag(0x00 = ed25519) || pubkey)
  uint8_t input[1 + 32];
  input[0] = 0x00;
  memcpy(input + 1, pub, 32);
...
bool loadTx(const String &line) {
  ...
  if (np < 3 || parts[0] != "SUI") return false;
...
String signTx(const uint8_t *entropy, size_t entLen, const char *passphrase) {
  ...
  // Sui always signs Blake2b-256(intent || bcs(TransactionData)), never the
  // raw bytes — unlike Substrate's length-conditional hashing.
  uint8_t digest[32];
  blake2b(digest, sizeof(digest), s_payload, s_payloadLen, nullptr, 0);
