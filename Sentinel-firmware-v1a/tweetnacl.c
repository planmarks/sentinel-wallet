...
/* Sentinel: expose ed25519 base-point scalar multiply for BIP32-Ed25519
 * (Cardano). Computes out = [scalar]B, scalar = 32-byte little-endian. */
void ed25519_scalarmult_base(u8 *out, const u8 *scalar)
{
  gf p[4];
  scalarbase(p, scalar);
  pack(out, p);
...
/* Sentinel: extended ed25519 signature (BIP32-Ed25519 / Cardano). Signs a
 * 32-byte message m using an extended key (kL scalar, kR nonce prefix) and the
 * public key A = [kL]B. Output sig = R(32) || S(32). */
void ed25519_sign_extended32(u8 *sig, const u8 *kL, const u8 *kR,
                             const u8 *A, const u8 *m)
{
  u8 buf[96], hh[64], rr[64];
  i64 x[64];
  gf p[4];
  int i, j;

  for (i = 0; i < 32; i++) buf[i] = kR[i];
  for (i = 0; i < 32; i++) buf[32 + i] = m[i];
  crypto_hash(rr, buf, 64);            /* r = SHA512(kR || m) */
  reduce(rr);
  scalarbase(p, rr); pack(sig, p);     /* R = [r]B */

  for (i = 0; i < 32; i++) buf[i] = sig[i];
  for (i = 0; i < 32; i++) buf[32 + i] = A[i];
  for (i = 0; i < 32; i++) buf[64 + i] = m[i];
  crypto_hash(hh, buf, 96);            /* h = SHA512(R || A || m) */
  reduce(hh);

  for (i = 0; i < 64; i++) x[i] = 0;   /* S = (r + h*kL) mod L */
  for (i = 0; i < 32; i++) x[i] = (i64) rr[i];
  for (i = 0; i < 32; i++) for (j = 0; j < 32; j++) x[i + j] += (i64) hh[i] * (i64) kL[j];
  modL(sig + 32, x);
}
