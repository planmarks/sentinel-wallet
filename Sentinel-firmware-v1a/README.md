# Sentinel — Open-Source Hardware Cold Wallet Firmware

**Sentinel** is firmware for a do-it-yourself hardware cryptocurrency wallet built around the
**Seeed XIAO ESP32-C5**, a 0.96" OLED display, and six buttons. It turns that small board into an
**offline transaction signer**: your private keys are generated on the device, encrypted, and never
leave it. A companion app (over USB serial or optional Bluetooth) prepares transactions, but every
signature has to be reviewed and physically confirmed on the device itself.
